...
class UploadMixin:
    ...
    def init_tus_upload(self, request):
        if request.method == 'OPTIONS':
            ...
            # find if cvat at prefix
            cvat_index = location.find('/cvat')
            if cvat_index == -1:    # if cvat not in prefix
                api_index = location.find('/api')   
                if api_index != -1: # add cvat before api
                    location = location[:api_index] + '/cvat' + location[api_index:]
                else:
                    print("Something went wrong in apps/engine/mixins.py")
            
            if 'HTTP_X_FORWARDED_HOST' not in request.META:
                # location = request.META.get('HTTP_ORIGIN') + request.META.get('PATH_INFO')
                location = request.META.get('HTTP_ORIGIN') + '/cvat' + request.META.get('PATH_INFO')
