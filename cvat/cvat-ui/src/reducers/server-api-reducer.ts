...
export default function (
    ...
    switch (action.type) {
        ...
        case ServerAPIActionTypes.GET_SERVER_API_SCHEMA_SUCCESS: {
            ...
            const isRegistrationEnabled = Object.keys(schema.paths).includes('/cvat/api/auth/register');
            const isBasicLoginEnabled = Object.keys(schema.paths).includes('/cvat/api/auth/login');
            const isPasswordResetEnabled = Object.keys(schema.paths).includes('/cvat/api/auth/password/reset');
            const isPasswordChangeEnabled = Object.keys(schema.paths).includes('/cvat/api/auth/password/change');
